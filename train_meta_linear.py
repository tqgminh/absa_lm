...
if __name__ == '__main__':
    ...
    log_file = f'meta-{args.dataset}-linear-blending-{strftime("%d%m%y-%H%M", localtime(time.time()+7*3600))}.log'
    ...
    meta_model = LinearClassifier(input_dim)
    ...
    for epoch in range(args.num_epoch):
        ...
        if max_f1_dev < total_f1_dev/len(dev_meta_data_loader):
            ...
            torch.save(meta_model.state_dict(), os.path.join(weight_folder, args.dataset, f'meta-{args.dataset}-blending-linear.pth'))
            logger.info(f'Save model weight !')
        
        logger.info('')
    
    meta_model.load_state_dict(torch.load(os.path.join(weight_folder, args.dataset, f'meta-{args.dataset}-blending-linear.pth'), map_location=device))
